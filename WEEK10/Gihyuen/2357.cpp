...
void make_max_segtree(int lo ,int hi , int idx){
    if(lo == hi){
        max_segtree[idx] = arr[lo];
        return;
    }
    int mid = (lo+hi)/2;
    make_max_segtree(lo,mid,idx*2);
    make_max_segtree(mid+1,hi,idx*2+1);

    max_segtree[idx] = max(max_segtree[idx*2], max_segtree[idx*2+1]);
    return ;
}
...
int get_max(int lo , int hi ,int l ,int r , int idx){
    if(l <= lo && hi <= r){
    ...
    int mid = (lo+hi)/2;
    return max(get_max(lo,mid,l,r,idx*2), get_max(mid+1,hi,l,r,idx*2+1));
...
void print(){
    ...
    for(int i = 1 ; i <= M ; i++){
        ...
        n1--;
        n2--;
        cout<<get_min(0,N-1,n1,n2,1)<<" "<<get_max(0,N-1,n1,n2,1)<<"\n";
    }
}
int main(){
    FastIO;
